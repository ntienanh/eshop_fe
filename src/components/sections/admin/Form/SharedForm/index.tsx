...
import { Badge, Box, Button, Card, Divider, Flex, Grid, Group, Stack, Text } from '@mantine/core';
...
import MediaDetail from '@/components/elements/formElements/MediaFormDetail';
...
import MediaFormDetail from '@/components/elements/formElements/MediaFormDetail';
...
const SharedForm = (props: IStudentFormProps) => {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <Box>
        <Button variant='subtle' leftSection={<IconArrowLeft size={20} />} onClick={() => router.back()}>
          Back
        </Button>
        ...
        <Flex justify={'space-between'} align={'center'} columnGap={12}>
          ...
          <Group>
            <Button type='submit' color='green' loading={isLoading} disabled={!isDirty && isValid}>
              Save
            </Button>
            {!isCreate && !!onDelete && (
        ...
        <Grid mt={24}>
          <Grid.Col span={9}>
            <Card>
              <Grid>
                ...
                <Grid.Col span={6}>
                  <SelectDetail name='select' control={control} label='Select' placeholder='Input Select' />
                </Grid.Col>
                ...
                <Grid.Col span={6}>
                  <MediaFormDetail name='logo' control={control} />
                </Grid.Col>
              </Grid>
            </Card>
          </Grid.Col>
          ...
          <Grid.Col span={3}>
            <Stack>
              <Card
                bg={isCreate ? 'var(--mantine-color-green-1)' : 'var(--mantine-color-blue-1)'}
                withBorder
                className={isCreate ? 'border-green-400 flex items-center' : 'border-blue-400 flex items-center'}
              >
                <Text
                  fw={700}
                  variant='gradient'
              ...
              <Card>
                <Stack>
                  ...
                  <Divider />
                  <Stack>
                    <Flex justify={'space-between'} align={'center'} columnGap={8}>
                      <Text fw={500}>Created</Text>
                      <Text>{!isCreate && data ? (dayjs(data?.createdAt) as any).fromNow() : '-'}</Text>
                    </Flex>

                    <Flex justify={'space-between'} align={'center'} columnGap={8}>
                      <Text fw={500}>Last update</Text>
                      {!isCreate && data ? (dayjs(data?.updatedAt) as any).fromNow() : '-'}
                    </Flex>

                    <Flex justify={'space-between'} align={'center'} columnGap={8}>
                      <Text fw={500}>Published At</Text>
                      {!isCreate && data ? (dayjs(data?.publishedAt) as any).fromNow() : '-'}
                    </Flex>
                  </Stack>
                </Stack>
              </Card>
